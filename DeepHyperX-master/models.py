...
from vit_pytorch.hit import HiT, WeightedPermuteMLP, ConvPermuteMLP
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        center_pixel = True
        kwargs.setdefault('epoch', 100)#100
        ...
        kwargs.setdefault("batch_size", 100)
    # elif name == "vit":
    #     kwargs.setdefault("patch_size", 13)
    #     center_pixel = True
    ...
    #     kwargs.setdefault("batch_size", 64)
        ...
        lr = kwargs.setdefault("learning_rate", 0.002)# 0.0003
        ### lr for efficient transformer
        #lr = kwargs.setdefault("learning_rate", 0.005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 13)
        center_pixel = True
        model = eftr.EftR(layers_ret1=1,
                           ffn1=3, heads1=1,faster_dim=42,hidden_dim=64,token_L=4,
                          dim2=None, num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)#有padd，lr0.0001；无padd，lr0.0007
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 64)
        ...
        kwargs.setdefault("patch_size", 13)
        center_pixel = True
        model = efr.efr(layers_ret1=1,
                           ffn1=3, heads1=1, faster_dim=40, hidden_dim=64,
                          dim2=None, num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)#有padd，lr0.0001；无padd，lr0.0007
        optimizer = optim.Adam(model.parameters(), lr=lr)
        ...
        center_pixel = True
        model = gaht.gaht(img_size=7,#(img_size=patch_size)
            n_groups=[2,2,2], depths=[1,1,1], num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        center_pixel = True
        model = LMST.lmst(img_size=11,#(img_size=patch_size)
                          num_stages=2,# if need ablation study of multi_scal3,num_stages=3
            n_groups=[2,2,2], depths=[1,1,1], num_heads=[8,8,8], num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        center_pixel = True
        model = ret3.ret3( patch_size=9, num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.0001)#0.002(~)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0001)#lr0.0001(~)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 64)
    elif name == "ret2":
        kwargs.setdefault("patch_size",7)
        center_pixel = True
        model = ret2.ret2( patch_size=7,num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)#0.002(~)有padd，lr0.0007。无pad，lr0.0008。
        optimizer = optim.Adam(model.parameters(), lr=lr)
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0001)#lr0.0001(~)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size",11)
        center_pixel = True
        model = ret2_cfr.ret2( patch_size=11, num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.0007)#0.002(~)有padd，lr0.0007。无pad，lr0.002。
        optimizer = optim.Adam(model.parameters(), lr=lr)
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0001)#lr0.0001(~)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size",11)
        center_pixel = True
        model = ret2_eft.ret2( patch_size=11, num_classes=n_classes, channels=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.0007)#0.002(~)有padd，lr0.0007。无pad，lr0.002。
        optimizer = optim.Adam(model.parameters(), lr=lr)
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0001)#lr0.0001(~)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = efctr.EFCTR(image_size=15, patch_size=3,
                          hidden_dim=80, ffn_size=240, heads=8,
                          dim2=None, num_classes=n_classes, channels=n_bands, faster_dim=42,
                          token_L=4, )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 9)
        center_pixel = True
        model = convformer.Setr_ConvFormer(
            n_channels=n_bands, n_classes=n_classes, imgsize=9, patch_num=3,)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 11)
        center_pixel = True
        model = eftt.EFTT(patch_size=11,faster_dim=40, hidden_dim=64, dim=64, channels=n_bands,
                    num_classes=n_classes )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 13)
        center_pixel = True
        model = ftr.FTR(hidden_dim=64, ffn=3, heads=1,
                           num_classes=n_classes, channels=n_bands, faster_dim=42,
                          token_L=4, )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 11)
        center_pixel = True
        model = ctr.CTR( ffn=3, heads=1,
                         faster_dim=42, hidden_dim=64,
                           num_classes=n_classes, channels=n_bands,
                          token_L=4,)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        ...
        kwargs.setdefault("patch_size", 11)
        center_pixel = True
        model = HybridEtAl(n_bands, n_classes, patch_size=11)
        lr = kwargs.setdefault("learning_rate", 0.001)#
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        lr = kwargs.setdefault("learning_rate", 0.001)#0.0001
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 64)
        ...
        lr = kwargs.setdefault("learning_rate", 0.0001)#0.0001
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 64)
...
#   from   models_hit
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        # embed_dims = [24, 24, 32, 32]  ## for IN 400, for GRSS 256, for PU 168, for KSC 320 for XA 480

        embed_dims = [984, 984, 1280, 1280]  ## for IN 400, for GRSS 256, for PU 168, for KSC 320 for XA 480
...
#         segment_dim = [8, 8, 4, 4]
#         mlp_ratios = [3, 3, 3, 3]
#         embed_dims = [368, 368, 512, 512]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
...
#         kwargs.setdefault("batch_size", 100)
        ...
        lr = kwargs.setdefault("learning_rate", 0.001)#0.003
        ...
        lr = kwargs.setdefault("learning_rate", 0.001)#0.01
        # optimizer = optim.SGD(
    ...
    def __init__(self, input_channels, n_classes, patch_size=5, dilation=1):
        ...
        if patch_size == 3:
            self.conv1 = nn.Conv3d(
                1, 20, (3, 3, 3), stride=(1, 1, 1), dilation=dilation, padding=1#dilation扩张率参数
            )
        else:
            self.conv1 = nn.Conv3d(
...
class HybridEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=15):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(8256, 64, (3, 3), padding=0, stride=(1, 1)),
...
def train(
    ...
    #epoch // 20 表示将epoch除以20并向下取整，得到保存时期的值。

    losses = np.zeros(1000000)
    mean_losses = np.zeros(100000000)
    iter_ = 1#用于迭代训练数据集的批次或迭代次数。
    gamma = 0.7
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            avg_loss += loss.item()
            losses[iter_] = loss.item()#iter_用于跟踪和操作迭代步骤的计数或索引。
            mean_losses[iter_] = np.mean(losses[max(0, iter_ - 100) : iter_ + 1])#这是一个列表切片操作，用于提取 losses 列表中从过去100个迭代步骤开始，直到当前迭代步骤的子列表。

            if display_iter and iter_ % display_iter == 0:#用于检查是否到达了一个应该显示训练进度的迭代步骤。
                string = "Train (epoch {}/{}) [{}/{} ({:.0f}%)]\tLoss: {:.6f}"
                string = string.format(
                    e,#第几个epoch
                    epoch,
                    batch_idx * len(data),#当前处理的样本数
                    len(data) * len(data_loader),#总样本数
                    100.0 * batch_idx / len(data_loader),#len(data_loader) 表示数据加载器（data_loader）中总共有多少个批次。
                    mean_losses[iter_],
                )
...
def test(net, img, hyperparams):
    ...
    probs = np.zeros(img.shape[:2] + (n_classes,))#(height, width, n_classes)
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            if patch_size == 1:
                data = [b[0][0, 0] for b in batch]#b[0]表示图像块的数据，而[0, 0]表示第一个通道的第一个像素。
                data = np.copy(data)
        ...
        with torch.no_grad():
            ...
            for out, pred in zip(output.view(-1), target.view(-1)):
                if out.item() in ignored_labels:
                    ...
                    accuracy += out.item() == pred.item()#输出值和目标值相等
                    total += 1
...
def applyPCA(X, numComponents=75):
    ...
    newX = np.asarray(np.copy(newX), dtype="float32")#原本为 dtype="float32"
    return newX