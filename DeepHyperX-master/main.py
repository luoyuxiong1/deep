...
from models import get_model, train, test, save_model, applyPCA
import get_cls_map
import argparse
RandomState = 345
...
parser.add_argument(
    "--dataset", type=str, default="GRSS", choices=dataset_names,
    help="Dataset to use   GRSS  PaviaU HyRANK  PaviaC HyRANK KSC Salinas whulk XA."
    #PaviaU 610x340 and 103 channels
    #GRSS 349x1905 and 144 channels
)
parser.add_argument(
    "--model",
    type=str,
...
group_dataset.add_argument(
    "--sampling_mode",
    type=str,
    help="Sampling mode" " (random sampling or disjoint, default: random   fixed)",
    default="random",
)
group_dataset.add_argument(
    "--train_set",
...
#appplypca
# img = applyPCA(img, numComponents=30)
# Number of classes
...
#padding0
# _, _, _, hyperparams = get_model(MODEL, **hyperparams)
#
#
# # padding the img
# print("..........................img{}".format(img.shape))
# img = padding_image(img, patch_size=hyperparams['patch_size'], constant_values=0)
# print(hyperparams['patch_size'])
#
# print("..........................after img{}".format(img.shape))
#
# #padding the gt
# gt = padding_gt(gt, patch_size=hyperparams['patch_size'], constant_values=0)
...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        train_gt, test_gt = sample_gt(gt, SAMPLE_PERCENTAGE, RandomState, mode=SAMPLING_MODE)
    print(
    ...
    gt_all = np.add(train_gt, test_gt)
    display_predictions(convert_to_color(gt_all), viz, caption="gt_all")
    ...
    if MODEL == "SVM_grid":
        ...
        X_train, y_train = build_dataset(img, train_gt, ignored_labels=IGNORED_LABELS)
        X_train, y_train = sklearn.utils.shuffle(X_train, y_train)#对训练集进行洗牌，以打乱样本的顺序。
        ...
        prediction = prediction.reshape(img.shape[:2])#将预测结果 prediction 重新调整为原始图像的形状，即 img.shape[:2]，以便与原始图像对应。
    elif MODEL == "nearest":
        X_train, y_train = build_dataset(img, train_gt, ignored_labels=IGNORED_LABELS)
        ...
        #train_gt, val_gt = sample_gt(train_gt, 0.95, RandomState, mode="random")
        #split_info_print(train_gt, val_gt, test_gt, LABEL_VALUES)
        split_info_print(train_gt, test_gt, LABEL_VALUES)


        # Generate the dataset
        train_dataset = HyperX(img, train_gt, **hyperparams)#img,整张图片
    ...
    # print(".............................")
    # print("color_prediction:  {}".format(color_prediction))
    ...
    #     caption=caption1,
    # )
    #viz.images(color_prediction)
    results.append(run_results)
    show_results(run_results, viz, label_values=LABEL_VALUES, txt_path=txt_path)
    print(".....prediction.shape{}".format(prediction.shape))

    prediction = remove_edge(prediction, hyperparams["patch_size"] // 2)
    print(".....prediction.shape{}".format(prediction.shape))
    map_dir = "./cls_result/" + DATASET + "/" +MODEL + "/"
    if not os.path.isdir(map_dir):
        os.makedirs(map_dir, exist_ok=True)

    map_path = map_dir + filename + ".png"
    map_path_gt = None
    print(type(prediction))
    print("...gt_all.shape{}".format(gt_all.shape))
    gt_all = remove_edge(gt_all, hyperparams["patch_size"] // 2)
    print(".....gt_all_after.shape{}".format(gt_all.shape))
    get_cls_map.get_cls_map(prediction,gt_all, map_path, map_path_gt)

# "./checkpoints/" + model_name + "/" + dataset_name + "/"
