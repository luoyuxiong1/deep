...
class ConvEtAl(torch.nn.Module):
    def __init__(self, input_channels, flatten=True):
        ...
        self.layer1 = nn.Sequential(collections.OrderedDict([
          ...
          ('relu',    nn.ReLU()),
          ('avgpool', nn.MaxPool2d(kernel_size=2, stride=2))#AvgPool2d
        ]))
        ...
        self.layer3 = nn.Sequential(collections.OrderedDict([
          ...
          ('relu',    nn.ReLU()),
          ('avgpool', nn.MaxPool2d(kernel_size=2, stride=2))#AvgPool2d  MaxPool2d
        ]))
    ...
    def forward(self, x):
        h = self.layer1(x)
        h = self.layer2(h)
        # print("after laly2 x {}".format(h.shape))

        h = self.layer3(h)
      #  h = self.layer4(h)
        #print(h.size())
        # print("after laly3 x {}".format(h.shape))
        if(self.is_flatten): h = self.flatten(h)
        # print("after flatten x {}".format(h.shape))
...
class TESTEtAl(nn.Module):
    ...
    def _get_sizes(self):
        ...
        x = self.feature(x)
        # print(" after feature x {}  ".format(x.shape))
        w, h = x.size()
        size0 = w * h
        return size0
