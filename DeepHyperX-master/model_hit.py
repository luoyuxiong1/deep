...
# # from vit_pytorch.conformer import Conformer
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        #optimizer = optim.Adadelta(model.parameters(), lr=lr, beta_1=0.9,beta_2=0.999,epsilon=1e-8)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        kwargs.setdefault("batch_size", 100)
    elif name == "swint":
        kwargs.setdefault("patch_size", 15)
        ...
        kwargs.setdefault("patch_size", 33)
        center_pixel = True
        model = FocalTransformer(img_size=33, embed_dim=128, depths=[2, 2, 18, 2], drop_path_rate=0.2,
                             focal_levels=[2, 2, 2, 2], expand_sizes=[3, 3, 3, 3], expand_layer="all",
                             num_heads=[4, 8, 16, 32],
                             focal_windows=[7, 5, 3, 1], window_size=3,
                             use_conv_embed=False, use_shift=False,
                             in_chans=n_bands, num_classes=n_classes,
                             )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
